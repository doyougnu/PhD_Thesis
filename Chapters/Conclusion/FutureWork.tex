...
variational solver. This format is likely to be inconvienient for end-users who
consider sets of \ac{sat} problems. Thus, a more complete approach would be to
input the set of \ac{sat} problems, then synthesize a \ac{vpl} formula with a
sharing ration that is \emph{good enough} and then run the solver on that
\ac{vpl} formula. In this section, we discuss an approach to synthesize a
\ac{vpl} formula
