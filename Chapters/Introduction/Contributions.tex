...
contributions of each chapter:
  ...
  \item \autoref{chapter:vpl} (\emph{Variational Propositional Logic})
    ...
    upon. This section makes several important contributions:
    \begin{enumerate}
    \item Introduces the essential aspects of variation using propositional
      logic and in the process presents the first instance of a recipe to
      construct a \emph{variation-aware} version of a system using the
      non-variational version.
    \item Defines and formalizes variational concepts that are used throughout
      the thesis, such as \emph{variant}, \emph{configuration} and
      \emph{variational artifact}.
    \item Proves theorems that are central to the soundness of variational
      \acl{sat}, such as the property of \emph{variation-preservation}
    \end{enumerate}


  \item  \autoref{chapter:vsat} (\emph{Variational Satisfiability Solving})
  \end{enumerate}
