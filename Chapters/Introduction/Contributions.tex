...
contributions of each chapter:
  ...
  \item \autoref{chapter:vpl} (\emph{Variational Propositional Logic})
    ...
    recipe to construct a \emph{variation-aware} a system using the
    ...
    Lastly, the section proves theorems that are central to proof of the
    soundness of variational satisfiability solving.
  ...
  \item \autoref{chapter:vsat} (\emph{Variational Satisfiability Solving}) makes
    ...
    general approach and architecture of a variational \acl{sat} as a system
    which uses a non-variational solver as a backend engine. This section
    provides a rationale for this design. Furthermore the section makes several
    important contributions:
    \begin{enumerate}
    \item A denotational semantics of variational satisfiability solving. A
      denotation of a variational \ac{sat} problem is a mapping from variational
      propositional logic to a program in the incremental \ac{sat} domain.
    \item A formal definition of concepts such as a \emph{variational core}
      which are transferable to domains other than \ac{sat}.
    \item A definition of the \emph{variational transpiler}  which may be used
      to create other variation-aware systems. The transpiler is defined in
      three phases that are ...
    \end{enumerate}
  \end{enumerate}
